...
import { cleanCart, removeCartItem } from "../redux/actions/cart"
...
const Cart = () => {
  ...
  const arrVariantPizza = [];
  const pizzasCart2 = Object.keys(items).map((key) => {
    items[key].items.map(index => {
      arrVariantPizza.push(index)
    });
  });
  const arrFilter = (idItem) => {
    return arrVariantPizza.filter(item => item.id === idItem)
  }


  const dispatch = useDispatch()
  const onClickCleanCart = () => {
    if (window.confirm('Clear cart?')) dispatch(cleanCart())
  }
...
<path d="M11.6666 9.16667V14.1667" stroke="#B6B6B6" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"/>
                ...
                <span onClick={onClickCleanCart}>Clear cart</span>
              </div>
            </div>
            <div className="content__items">
              {pizzasCart.map(pizza => 
                <CartItem 
                  arrayVariations={arrFilter}
                  onRemoveItem={() => dispatch(removeCartItem(pizza.id))} 
